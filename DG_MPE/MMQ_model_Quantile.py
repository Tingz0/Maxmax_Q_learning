...
class MMQ_Q_Agent():
    def __init__(self, args, id = None): #lr = 3e-4
        ...
        for i in range(self.n_ensemble):
            if self.args.env in [ "DG"] :
                self.forward_model_l.append(Quantile_model_DG(args.state_space, args.action_dim, Q_hidden_dim=args.quantile_hidden_dim).to(self.device))
                self.forward_model_u.append(Quantile_model_DG(args.state_space, args.action_dim, Q_hidden_dim=args.quantile_hidden_dim).to(self.device))
    ...
    def train(self, id, total_steps, buffer, batch_size = 100):
        ...
        if  ( total_steps > self.args.start_timesteps):
            for ee in range(self.n_ensemble):
                if self.args.random_batch:
                    ...
                    L_ns  = self.forward_model_l[ee](states[mini_index], actions[mini_index])
                    U_ns = self.forward_model_u[ee](states[mini_index], actions[mini_index])
                    forward_model_loss_l = self.quantile_loss(L_ns, next_states[mini_index], tau = 0.05 )
                    forward_model_loss_u = self.quantile_loss(U_ns, next_states[mini_index], tau = 0.95 )
                else:
        ...
        if ( total_steps > self.args.start_timesteps):
            ...
            all_predict_US = torch.zeros((batch_size, self.args.state_space, self.n_ensemble))
            all_predict_LS = torch.zeros((batch_size, self.args.state_space, self.n_ensemble))
            ...
            if (not self.args.sample_from_avg) and (not self.args.sample_from_bound):
                ...
                for ee in range(self.n_ensemble):
                    ...
                    all_predict_US[:, :,  ee] = copy.deepcopy(pred_us.detach())
                    all_predict_LS[:, :, ee] = copy.deepcopy(pred_ls.detach())
                B_US = torch.max(all_predict_US, dim = -1)[0] # shape = [batch_size, state_space]
            ...
            predict_US = torch.max(all_predict_US, dim = -1)[0] # shape = [batch_size, state_space]
            predict_LS = torch.min(all_predict_LS,  dim = -1)[0]
            precent_in, mean_jud = self.declare_in(predict_US, predict_LS, next_states)

            large_dis, small_dis = self.compute_dis_bound(predict_US, predict_LS, next_states)

            if self.args.sample_from_avg:
                precent_in_avg, mean_jud_avg = self.declare_in(avg_US, avg_LS, next_states)
                large_dis_avg, small_dis_avg = self.compute_dis_bound(avg_US, avg_LS, next_states)


            if self.args.use_wandb and id == 0:
                wandb.log({f'Agent_{id}_Precentage of next hidden state in quantile predicted bound': precent_in}, step=total_steps)
                ...
                wandb.log({f'Agent_{id}_mean dimension in quantile predicted bound': mean_jud}, step=total_steps)
                wandb.log({f'Agent_{id}_mean distance between larger ns and upper bound': large_dis}, step=total_steps)
                wandb.log({f'Agent_{id}_mean distance between smaller ns and lower bound': small_dis}, step=total_steps)


                if self.args.sample_from_avg:
                    wandb.log({f'Agent_{id}_Precentage of next state in AVERAGE quantile predicted bound': precent_in_avg}, step=total_steps)
                    wandb.log({f'Agent_{id}_mean dimension in AVERAGE quantile predicted bound': mean_jud_avg}, step=total_steps)
                    wandb.log({f'Agent_{id}_mean distance between larger ns and AVERAGE upper bound': large_dis_avg}, step=total_steps)
                    wandb.log({f'Agent_{id}_mean distance between smaller ns and AVERAGE lower bound': small_dis_avg}, step=total_steps)
    ...
    def save_buffer(self, buffer, size = 1000):
        ...
        experience_dict = {'states':states, 'actions':actions, 'rewards':rewards, 'next_states':next_states, 'dones': dones}
        return experience_dict
    
    def declare_in(self, pred_quan_u, pred_quan_l, next_states): # next_states shape = [batch_size, state_space]
        ...
        jud = ( next_states <= pred_quan_u  ) & (next_states >= pred_quan_l) # if == 1, then is in;  [batch_size, state_space ]
        jud = torch.sum(jud, dim = -1)
        jud_all = ( jud >=  self.args.state_space) # if in, 1; otherwise, 0 shape [batch_size]
    ...
    def compute_dis_bound(self, pred_quan_u, pred_quan_l, next_states):
        pred_quan_u = torch.FloatTensor(pred_quan_u).to(self.device)
        pred_quan_l = torch.FloatTensor(pred_quan_l).to(self.device)
        jud_large = next_states > pred_quan_u # [batch_size, state_space] record each dimension and compare with pred_quan_u 
    ...
    def save(self, filename, agent_idx):
        ...
        for j in range(self.args.n_ensemble):
            torch.save(self.forward_model_l[j].state_dict(), filename + "_" + str(agent_idx) + f"_forward_model_l{j}")
            torch.save(self.forward_model_u[j].state_dict(), filename + "_" + str(agent_idx) + f"_forward_model_u{j}")
        if self.args.learn_reward:
    ...
    def load(self, filename, agent_idx):
        ...
        for j in range(self.args.n_ensemble):
            self.forward_model_l[j].load_state_dict(torch.load(filename + "_" + str(agent_idx) + f"_forward_model_l{j}"))
            self.forward_model_u[j].load_state_dict(torch.load(filename + "_" + str(agent_idx) + f"_forward_model_u{j}"))
        if self.args.learn_reward:
